...
class ExpenseSyncService {
  ...
  // ── PUSH ONE PENDING EXPENSE ───────────────────────────────
  Future<Map<String, dynamic>> _pushExpense(
    Expense expense,
    String authToken,
  ) async {
    try {
      _log('Pushing expense: ${expense.title} (localId: ${expense.id})');
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (result['success'] == true) {
          // Delete the old local PENDING copy (UUID)
          await _db.deleteExpense(expense.id);
          _log('✅ Deleted local PENDING copy: ${expense.id}');

          return {'success': true};
        }
      }

      _err('Push failed: ${response.statusCode} ${response.body}');
  ...
  Future<List<Expense>> fetchAndSyncGroupExpenses(
    ...
    try {
      _log('Fetching expenses for group $groupId from backend');
      ...
      if (response.statusCode == 200) {
        ...
        if (result['success'] == true) {
          ...
          _log('Received ${list.length} expenses from backend');
